...
/**
 * This class stores the customers data, creates customer accounts
 * if needed and gives benefits to the customers with accounts.
 * 
		...
		if (ans.equalsIgnoreCase("yes")) {
			...
			int reasonfortravel = 0;
				...
				do {
					...
					if (satisfuction != 1 && satisfuction != 2 && satisfuction != 3 &&
						satisfuction !=4 && satisfuction !=5	) {
						System.out.println("Insert 1 or 2 or 3! ");
					}
				} while  (satisfuction != 1 && satisfuction != 2 && satisfuction != 3 &&
						satisfuction !=4 && satisfuction !=5	);
			...
			for (int i = 0; i <= AccountCustomers.getCounteracc(); i++) {
				...
				if (t.getUsername() == usr) {
					if (t.getPassword() == pswrd) {
						...
						t.setPayment(t.getTotalPayment() + 
								payment -  t.getMembershipCode().beneffitsPay()*payment);
