...
/**
 * 
 * @author Ioannis Alexios Perakis, Aggeliki Nina Kafouni With the use of this
 *         class the user can compare profits among the past 3 years. Moreover,
 *         user can set each year's goals and by pressing 3 can compare current
 *         year's profits and the goal of the month that have been chosen.
 */
...
public class Goals {
	...
	public static void comparison(int ans, int month) { // ans einai to year pou thelei na sigrinei
		...
		try {
			if (ans == 1) {
				...
				System.out.println(
						"Currents profits: " + ReportingFinance.ProfLosBase[month - 1] + "\nGoal: " + goals[month - 1]);
				System.out.println();
			}
	...
	public static void loadgoals(double ja, double feb, double mar, double ap, double may, double june, double july,
			double au, double se, double oc, double no, double dec) {
