...
/**
 * The thread that does automatically the actions need to be done at the start
 * of every week (Monday). *** WON'T be used at the presentation of the project
 * because of the difficulties at checking.
 * 
...
public class weekThread implements Runnable {
	...
	/**
	 * run method of the thread, calls the methods that contain the actions need to
	 * be done at the start of every week after every execution of these actions
	 * sleeps till the next Monday (at 00:00)
	 */
	public void run() {
		while (true) {
			try {
