			...
			if (ans.equalsIgnoreCase("no")) {
				...
				if (an.equalsIgnoreCase("no")) {
					...
					System.out.println("You are " + ac.getMembershipCode() + " member!"
							+ "\n That means you have a " + ac.getMembershipCode().beneffitsPay()*100 + "% discount");
					return (payment);
				}
			} else {
					...
					for (AccountCustomers t : AccountCustomers.accCustomerBase) {
						...
						if (t.getUsername().equals(usr)) {
							if (t.getPassword().equals(pswrd)) {
								...
								t.setMembershipCode(MembershipSelection(payment));
								t.setPayment(
							    t.getTotalPayment() + payment - t.getMembershipCode().beneffitsPay() * payment);
								System.out.println("You are " + t.getMembershipCode() + " member!"
										+ "\n That means you have a " + t.getMembershipCode().beneffitsPay()*100 + " % discount");
